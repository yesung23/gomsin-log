...
| 하루 격자 (`MonthGrid` · `monthTexture`) | **`우리` 첫 탭이 게시물 격자가 되면서 마운트되지 않는다 (2026-08-23).** 한 달의 모든 날을 7열로 그리던 것이고, "달력이 아니라 질감" 이라는 의도와 달리 쓰는 사람에게는 달력으로 읽혔다. 칸의 단위가 하루에서 **기록**으로 바뀌면서 하루 격자를 고른 원래 걱정(사진 안 올리는 커플에게 구멍 난 격자)이 구조적으로 사라졌다. 컴포넌트와 `monthTexture` 는 저장소에 그대로 있으므로 되돌리려면 자리만 정하면 된다 |
