...
### Memory Product 실물 제작·배송 경계 (2026-08-15 추가)

실물 기억상품(POD) 제작은 두 가지 이유로 통상적인 E2EE 애플리케이션 경계를 벗어난다.
사용자가 명시적으로 고른 콘텐츠가 제작을 위해 외부로 나가고, 배송을 위해 새로운
개인정보(수령인 이름·주소·연락처)가 필요해진다.

PRODUCT PRIVACY DECISION — 아래는 이 문서가 소유하는 데이터 경계 규칙이다.

- 사용자의 명시적 행위로 선택된 콘텐츠만 경계를 벗어난다. 아카이브 전체를 제작·외부
  처리 대상으로 보내지 않는다.
- 제작·배송에 필요한 최소 항목만 수집한다.
- 목적 제한: 배송정보는 해당 주문 제작·배송·CS 목적 밖으로 사용하지 않는다.
- 외부 제작·배송 처리자에게 전달되는 범위를 주문 단위로 한정하고 명시한다.
- 보관·삭제 처리를 주문 완료 기준으로 정한다.
- 배송정보는 관계 콘텐츠 키(PMK/CSK/HRK)와 섞지 않는다. 건강 데이터는 어떤 경우에도
  제작·배송 경로에 포함하지 않는다.
- 사용자에게 어떤 콘텐츠와 어떤 개인정보가 나가는지 결제 전에 표시한다.

**LEGAL SPECIALIST FOLLOW-UP REQUIRED.** 처리위탁 계약 요건, 국외 이전 해당 여부,
법정 보관·파기 기간, 전자상거래 관련 의무는 이 문서에서 판단하지 않았다. 실제 배송
개시 전에 별도 법률 검토가 필요하며, 여기서 준수 여부를 주장하지 않는다. 사업 관점의
리스크는 [`BUSINESS_MEMORY_ROADMAP_V1.md`](BUSINESS_MEMORY_ROADMAP_V1.md) §13이 소유한다.
