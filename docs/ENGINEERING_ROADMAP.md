...
현재 UI 실행 우선순위는 사용자가 승인한 2026-09-05 종이 노트 이미지와
"기존 작업을 master에 먼저 올린 뒤 시작" 지시를 따른다. 2026-09-06 PR #93 코드 통합을 마쳤으므로
실제 Home 한 화면의 가로/세로 읽기·긴 글/사진·light/dark·접근성 검증을 먼저 수행한다.
이후 별도 책 북마크·요약 원본 연결·날짜별 기록 표면을 확장한다. Book Studio 자체는 별도 task 소유다.
이 우선순위 변경은 승인 이미지의 실제 화면 구현을 앞당기기 위한 것이며, 운영 DB/복구·로그인·실기기·
보안/배포 gate를 없애거나 master 코드 통합을 RC 완료로 바꾸지 않는다.
