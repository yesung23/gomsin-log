...
| **P4** | **채팅 제품/데이터 계약 및 구현 계획** → [`CHAT_PRODUCT_DATA_CONTRACT_V1.md`](CHAT_PRODUCT_DATA_CONTRACT_V1.md) **(계약 작성 완료, 구현 미착수)** | 채팅은 코어이며 사용자 콘텐츠다. E2EE 수직 슬라이스를 시작하기 **전에** 계약을 확정해야, daily_records와 채팅이 같은 봉투/키 경로를 두 번 설계하지 않는다. 계약 §17이 구현 전 닫아야 할 게이트 6개(G1–G6)를 정의한다 |
