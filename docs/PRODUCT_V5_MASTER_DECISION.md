...
### 2026-09-05 추가 승인 — 시간 기반 복무 여정

최신 사용자 요청으로 찾기·복무 상세에 계급 단계와 실시간 EXP를 제공한다. 과거 EXP 제거
결정을 이 범위에서 대체한다. 같은 날의 후속 명시적 승인으로 느린 Lv.200 대신
**1초=10 EXP, 36,000 EXP=1레벨(실제 복무 한 시간)**의 짧은 성장을 사용한다.
시작은 Lv.1, 전역 시 상한은 전체 복무 시간 수+1이며 대기 중에는 Lv.0이다.
접속·기록·열람·구매는 경험치에 영향을 주지 않는다. 군 계급은 인사정보가
아닌 **예상 앱 단계**다. 사회복무·기타는 군 계급 없는 중립 여정을 제공한다.
일초·일꺾·일말·상초·상꺾·상말·병초·왕고·말년은 해당 예상 계급 구간을 나눈
재미용 칭호이며, 실제 선임 관계를 증명하지 않는다. 직접 도달한 레벨 변화는 짧게
보여 주되 접속하지 않은 시간도 동일하게 성장하며 보상을 받기 위한 접속은 필요 없다.
시간 표시 일시정지/동작 줄이기/화면 밖·백그라운드 중지, 날짜 모순 복구를 지원한다.
XP를 결제 재화·관계 점수로 전환하지 않는다. 날짜나 기기 시각 수정에 따라 표시도 다시
계산되므로 서버가 증명한 점수나 실제 진급 증빙으로 사용하지 않는다.
...
AI는 원문 대신 무엇이 중요하다고 고르지 않는다. 먼저 권한이 확인된 적격 공유 기록 전체를
시간순으로 규칙 기반 구성하고, 사용자가 버튼을 누를 때 **같은 항목 수·같은 순서·같은
원본 연결을 유지한 채 각 기록의 문장만 짧게 정리**한다. 요약 한 줄은 항상 기록 한 건에
결속되고, 누르면 그 문장의 정확한 원본 기록으로 이동한다.

- 모델 입력에 record id, 사용자 id, 커플 id, 날짜·시각, 첨부 URL·파일명, 위치 metadata를
  넣지 않는다.
- `isPrivate === false`이고 현재 active partner가 실제로 읽을 수 있는 공유 기록 본문만
  입력한다. 비공개·권한 없음·복호화 불가·현재 파트너가 아닌 기록은 fail-closed한다.
- 사용자가 공유 기록 본문에 직접 적은 건강·주기·성적 내용이나 정확한 장소 표현은
  **온디바이스 입력에서 일괄 검열하지 않는다.** 이는 새 위치 수집이나 건강정보 자동 공유
  승인이 아니다. 소유자 전용 구조화 주기·건강 테이블, GPS/EXIF, 위치 metadata는 여전히
  입력하지 않는다.
- 모델은 같은 기록에서 나온 원문 발췌만 만들고, 기록 선택·병합·추가·누락·재배열 또는
  의역을 하지 않는다. 숫자·시각·고유명사·부정·불확실성·질문·강조를 새로 만들거나
  바꾸지 않는다.
- 입력 일부를 생략한 발췌는 화면에서 생략 표시를 숨기지 않는다. 사용자는 한 번 눌러 전체
  원문과 맥락을 확인할 수 있다.
- 한 항목이라도 출처 검증에 실패하면 앞서 성공한 결과까지 버리고 그날 전체의 규칙 결과를
  유지한다. cloud AI fallback은 두지 않는다.

첫 실기기 후보 계약은 `기록당 최대 120 UTF-16 단위 원문 → 최대 40 단위 원문 발췌`, 호출당
최대 5개, 하루 최대 20개, 배치당 4초다. 20개를 넘거나 어느 배치든 실패하면 일부 기록만 AI
문장으로 바꾸지 않고 모든 기록을 규칙 요약으로 보여 준다. 이 수치는 실물 iPhone의 한국어
품질·지연·메모리·발열·배터리 결과에 따라 낮출 수 있으며, 검증 전에는 구현 완료나 출시
활성화를 뜻하지 않는다.
...
않고 같은 규칙 요약을 사용한다. 온디바이스 처리는 앱 서버 전송을 줄이는 경계이지 E2EE나
운영체제 수준의 완전한 비노출을 보증하는 표현이 아니므로, 사용자 고지와 실기기 네트워크
검증을 활성화 gate에 포함한다.
