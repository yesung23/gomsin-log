# APP_NAME (곰신로그) Supabase 배포 및 운영 가이드

본 문서는 **`APP_NAME` (군화와 곰신을 위한 1:1 비공개 데일리 로그 서비스)**의 Supabase 백엔드 연동, 데이터베이스 마이그레이션, RLS 보안 정책, Storage 설계, 및 웹 호스팅(Vercel/Netlify) 배포 절차를 안내하는 운영 문서입니다.
