...
- 상태: **기반 + 화면 재설계 완료, 검증 미완료** — 16개 작업 중 10개 완료
...
코드 작업은 끝났습니다. **남은 것은 검증과 마무리뿐입니다.**

1. Playwright e2e 실행 (`node node_modules/@playwright/test/cli.js test --config playwright.config.ts`)
2. 320 / 390 / 430px × 라이트 / 다크 스윕, 글자 확대, safe-area 확인
3. `design-preview/capture.mjs`로 스크린샷 캡처 후 **PNG를 실제로 열어** `DESIGN_V2.md`의
   `시각 수용 기준` 절과 대조
4. `DESIGN_V2.md`에 레퍼런스 경로와 적용 원칙 기록
5. PR CI green 확인 후 **merge commit**으로 병합
6. 비기술 최종 보고서

특히 **아직 아무도 눈으로 확인하지 않았습니다.** 테스트는 전부 통과하지만 실제 화면이
의도대로 보이는지는 미확인입니다. 3번이 가장 중요합니다.

확인이 필요한 강한 수용 기준 2개:
- 390×844 군화 홈에서 브리핑 완료 버튼과 상대의 실제 기록 일부가 **스크롤 없이 동시에** 보이는가
- 390×844 곰신 홈에서 기록 유형 선택 + 오늘의 실제 기록 일부 + 축소형 D-Day가 보이는가
...
| 11 | 밀도·색상 변경에 영향받는 테스트 갱신 | **완료** — 유닛 1294개 전부 통과 |
| 12 | e2e + 320/390/430 + 라이트/다크 검증 | **미완료** ← 여기서부터 |
| 13 | 캡처 스크린샷을 수용 기준과 대조하는 시각 리뷰 | **미완료** |
| 14 | 레퍼런스 경로와 적용 원칙을 디자인 문서에 기록 | 미완료 |
