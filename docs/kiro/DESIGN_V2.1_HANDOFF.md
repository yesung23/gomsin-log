...
- 상태: **기반 + 화면 재설계 + 시각 검증 완료** — 16개 작업 중 14개 완료 (남은 것: 병합, 파일럿 사용자 평가)
...
**2026-08-09 갱신: 검증이 끝났습니다.** 남은 것은 병합과 파일럿뿐입니다.

1. PR CI green 확인 후 **merge commit**으로 병합 (squash·rebase 금지)
2. 파일럿에서 대표 사용자 평가 수집 — 코드로 판정할 수 없는 마지막 수용 기준

검증에서 확인된 것과 고친 것은 `DESIGN_V2.md`의 `시각 수용 기준 대조 결과 (2026-08-09)`
절에 있습니다. 요약하면 강한 수용 기준 2개는 **모두 통과**했고, 대조 과정에서 실제 회귀
3건을 찾아 고쳤습니다: 기록 본문 행간이 1.4 하한 밑(1.375)으로 눌려 있었고, 홈 헤더에서
로그인한 사람의 이름이 사라져 있었고, 위젯 추가 버튼의 접근 이름이 바뀌어 e2e 훅이
끊겨 있었습니다.

### 이 저장소에서 검증을 재현하는 방법 (2026-08-09 실측)

- **유닛 기준선은 1298개입니다** (가드 2개 추가). 이 문서가 앞서 적은 “1294개 전부 통과”는
  Node 26에서 재현되지 않습니다. Node 26이 `localStorage`를 실험적 전역으로 선점해
  jsdom의 것을 가리므로, 플래그 없이 실행하면 98파일 1294개가 **전부** 실패합니다.
  ```
  NODE_OPTIONS="--localstorage-file=<임의 경로>" node node_modules/vitest/vitest.mjs run --config vitest.config.ts --configLoader runner
  ```
  이 플래그를 주면 1296/1298 통과합니다. 남는 2건(`accountDeletion` C1,
  `accountDeletionRecovery` 8)은 **master에서도 동일하게 실패**하므로 이 브랜치의 회귀가
  아닙니다. 같은 Node 26 저장소 의미 변화 때문이며, 디자인과 무관한 별도 항목입니다.
- **e2e는 54개 전부 통과합니다.** Playwright 관리 Chromium이 없는 환경에서는 시스템 Chrome을
  쓰도록 `channel: 'chrome'`만 덮어쓴 로컬 설정으로 실행할 수 있습니다. 뷰포트와
  `webServer`는 커밋된 설정과 동일해야 합니다.
- **캡처는 152 프레임입니다.** `design-preview/capture.mjs`는 18화면 × 320/390 × 라이트/다크
  × 상태 변형을 찍고 매 조합의 가로 오버플로를 함께 감사합니다. 현재 전부 오버플로 없음.
...
| # | 작업 | 상태 |
| --- | --- | --- |
| 5 | 곰신 홈 재설계 | **완료** (`244066c`) |
| 6 | 군화 홈 재설계 | **완료** (`244066c`) |
| 7 | 기록 타임라인을 에디토리얼 타임라인으로 | **완료** (`244066c`) |
| 8 | 일정 + 공유 할 일을 시간순 행으로 | **완료** (`244066c`) |
| 9 | 여행(목록·상세)을 컴팩트 일정표로 | **완료** (`244066c`) |
| 10 | 온보딩·우리·서비스·마이·설정에 같은 언어 적용 | **완료** (`244066c`) |
| 11 | 밀도·색상 변경에 영향받는 테스트 갱신 | **완료** — 유닛 1298개 (가드 2개 추가) |
| 12 | e2e + 320/390/430 + 라이트/다크 검증 | **완료** — 실제 Chrome에서 54/54 통과 |
| 13 | 캡처 스크린샷을 수용 기준과 대조하는 시각 리뷰 | **완료** — 152 프레임, 회귀 3건 발견·수정 |
| 14 | 레퍼런스 경로와 적용 원칙을 디자인 문서에 기록 | **완료** — `DESIGN_V2.md` §레퍼런스에서 무엇을 가져왔고 무엇을 버렸는가 |
| 15 | PR, CI green 확인 후 merge commit 병합 | PR 생성됨, 병합 전 |
| 16 | 비기술 최종 보고서 | 미완료 |

### 커밋 `244066c` — 화면 재설계

- **기록** — 채팅 말풍선 타임라인을 에디토리얼 행으로. 시간 열 44px 고정, 미디어 열
  68/76px, 그다음 사용자 원문. 열기 버튼은 여전히 `자세히 보기`로 끝나는 실제 버튼이고,
  미디어는 absolute 형제로 두어 `<video controls>`가 버튼 안에 들어가지 않으며 시간
  클릭으로 상세가 열리는 동작도 유지된다.
- **홈** — 큰 타일 4개 대신 컴팩트 캡처 런처. 브리핑은 결정 표면으로 축소, 상대의 실제
  기록이 위로 올라옴. 기본값에서 내려온 위젯 4개는 코드 전부 유지.
- **일정·여행** — 일정·할 일·여행·장소가 시간이 먼저 읽히는 시간순 행으로. OCR·링크·
  체크리스트·자동/수동 정렬 동작 변경 없음.
- **계정 화면** — 온보딩·우리·서비스·마이·설정을 리스트 행으로, 화면당 주요 결정 1개,
  파괴적 행동은 별도 그룹.

**작성자 구분 채널 교체** — 3중 채널 중 가운데(`ml-auto`/`mr-auto` + `max-w-[94%]`)는
고정 시간 열과 공존할 수 없어 **버리지 않고 교체**했다. 소유권 기준 마커 모양(내 기록은
채운 점, 상대는 빈 링)이며, 역할 스트라이프와 독립적으로 유지되도록 강조색이 아니라
잉크(`bg-foreground` / `border-foreground`)를 쓴다. 색상은 여전히 유일한 신호가 아니다.

**갱신한 테스트 5건 (약화 아님)**

- `recordAuthorDistinction` — 두 작성자 비교를 wrapper의 `className`이 아니라 행 전체의
  class 시그니처로. 채널이 자손으로 옮겨갔기 때문이며, 이름 텍스트만 다를 때 실패하므로
  원래보다 강하다. 칩 탐색은 `·`를 **직접 렌더하는** 요소를 찾도록 정밀화(행 wrapper가
  버튼 안이라 span이 되었고 조상 span이 먼저 매칭되던 문제).
- `settingsRouteReachability` — 44px 보장이 `PressableRow` 프리미티브로 이동했으므로
  행마다의 유틸리티가 아니라 프리미티브를 검사한다.
- `themeTokens` C6 — 리터럴 `min-h-[44px]`/`min-w-[44px]`와 함께 `min-h-11`(=44px)과
  `w-full`을 허용. 규칙은 그대로이고 허용 표기만 넓혔다.
- `renderedTypeScale` — 하한 13→12px, 기록 본문 16→15px, 행간 1.4 이상 유지.
- `DDayWidget` — 불필요한 문구 변경을 되돌렸다. `기념일 미설정`은 UsPage가 같은 사실에
  쓰는 리터럴이고 두 테스트가 두 화면의 일치를 검증한다.

**검증 결과 (실제 실행함)**: typecheck 0, eslint 0, 유닛 98파일 / 1294개 전부 통과.
