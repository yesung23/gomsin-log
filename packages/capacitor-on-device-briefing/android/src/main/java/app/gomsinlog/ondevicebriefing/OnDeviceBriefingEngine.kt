...
/**
 * Data structures representing candidate extractions, items, and choices.
...
object OnDeviceBriefing {
    ...
    const val INSTRUCTIONS = "Choose one supplied candidate for every item. Return only itemOrdinal and candidateOrdinal in JSON format: {\"version\":1,\"choices\":[{\"itemOrdinal\":0,\"candidateOrdinal\":0}]}. Keep every item once and in order. Never write markdown, commentary, or text outside the JSON object."
 ...
 */
class OnDeviceBriefingEngine(private val scope: CoroutineScope) {
    ...
    suspend fun select(
        ...
        val deferred = CompletableDeferred<List<BriefingChoice>>()
        val job = scope.launch(start = CoroutineStart.LAZY) {
            try {
                val choices = runMlKitInference(items, itemsJSON)
        ...
        val existing = inFlight.putIfAbsent(requestId, job)
        if (existing != null) {
            job.cancel()
        ...
        try {
            ...
            throw OnDeviceBriefingException(BriefingErrorCode.CANCELLED)
        } finally {
            inFlight.remove(requestId, job)
        }
    }
    ...
    private suspend fun runMlKitInference(
        ...
        try {
            ...
            return parseAndValidateChoices(rawOutput, items)
        } finally {
            model.close()
        }
    }

    private fun parseAndValidateChoices(
        rawOutput: String,
        items: List<BriefingItem>
    ): List<BriefingChoice> {
        ...
        if (topKeys != setOf("version", "choices")) {
            throw OnDeviceBriefingException(BriefingErrorCode.MALFORMED)
        }
        ...
        if ((versionVal as Number).toInt() != 1) {
            throw OnDeviceBriefingException(BriefingErrorCode.MALFORMED)
        }

        val choicesArray = json.optJSONArray("choices")
            ?: throw OnDeviceBriefingException(BriefingErrorCode.MALFORMED)
        if (choicesArray.length() != items.size) {
            throw OnDeviceBriefingException(BriefingErrorCode.MALFORMED)
        }

        val result = ArrayList<BriefingChoice>(items.size)
        for (i in 0 until choicesArray.length()) {
            val itemObj = choicesArray.optJSONObject(i)
                ?: throw OnDeviceBriefingException(BriefingErrorCode.MALFORMED)
            val choiceKeys = itemObj.keys().asSequence().toSet()
            if (choiceKeys != setOf("itemOrdinal", "candidateOrdinal")) {
                throw OnDeviceBriefingException(BriefingErrorCode.MALFORMED)
            }

            if (!itemObj.has("itemOrdinal") || !itemObj.has("candidateOrdinal")) {
                throw OnDeviceBriefingException(BriefingErrorCode.MALFORMED)
            }

            val itemOrdinalVal = itemObj.opt("itemOrdinal")
            val candidateOrdinalVal = itemObj.opt("candidateOrdinal")
            ...
            val candidateCount = items[i].candidates.size
            if (candidateOrdinal < 0 || candidateOrdinal >= candidateCount) {
                throw OnDeviceBriefingException(BriefingErrorCode.MALFORMED)
            }

            result.add(BriefingChoice(itemOrdinal, candidateOrdinal))
        }
