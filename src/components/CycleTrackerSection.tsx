  ...
  return (
    <section className="bg-card rounded-3xl p-5 border border-border shadow-sm space-y-4">
      <div className="flex items-center justify-between border-b border-border/40 pb-3 gap-2">
        ...
        <span className="text-[10px] text-coral font-bold bg-coral/10 px-2.5 py-1 rounded-full whitespace-nowrap">
      ...
      {loadState === 'loading' && (
        <div className="py-10 flex items-center justify-center gap-2 text-xs text-muted-foreground" role="status">
        ...
        <>
          {expectedStartDate ? (
            <div className="p-4 rounded-2xl bg-coral/10 border border-coral/20 space-y-1">
              <div className="flex items-center justify-between gap-2">
                <span className="text-[11px] text-coral font-bold">다음 예상 시작일</span>
                <span className="text-sm font-extrabold text-foreground">{expectedStartDate}</span>
              </div>
          ...
          <div className="space-y-3 pt-1">
            <div className="flex items-center justify-between px-1">
              ...
              <span className="text-xs font-bold text-foreground">{viewYear}년 {viewMonth + 1}월</span>
            ...
            <div className="grid grid-cols-7 text-center text-xs gap-1 font-medium">
              {cells.map((cell, index) => {
                ...
                return (
                  ...
                  >
                    <span>{cell.day}</span>
                    {hasRange && <span className="text-[8px] leading-none mt-0.5">{hasStart ? '시작' : '기간'}</span>}
                  </button>
                );
              })}
            </div>
          ...
          <div className="rounded-2xl border border-border bg-muted/20 p-4 space-y-3">
            ...
            {selectedMatches.length === 0 ? (
              <p className="text-[11px] text-muted-foreground text-center py-2">이 날에 해당하는 기록이 없어요.</p>
            ) : selectedMatches.map(({ entry }) => (
              <div key={entry.id} className="bg-card border border-border rounded-xl p-3 space-y-2">
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <p className="text-xs font-bold text-foreground">{formatRange(entry)}</p>
                    {entry.symptoms.length > 0 && (
                      <p className="text-[10px] text-muted-foreground mt-1">{entry.symptoms.map((item) => symptomLabels[item]).join(' · ')}</p>
                    )}
                  </div>
                ...
                {entry.notes && <p className="text-[11px] text-foreground whitespace-pre-wrap">{entry.notes}</p>}
              </div>
            ))}
          </div>
          ...
          {loadState === 'empty' && (
            <div className="p-4 rounded-2xl border border-dashed border-border text-center space-y-1">
              ...
              <p className="text-xs font-bold text-foreground">아직 저장한 기록이 없어요.</p>
              <p className="text-[10px] text-muted-foreground">날짜를 선택하고 첫 기록을 추가해 보세요.</p>
            </div>
          )}
          ...
          {formOpen && (
            <div className="rounded-2xl border border-coral/30 bg-coral/5 p-4 space-y-3">
              <div className="flex items-center justify-between">
                <h4 className="text-xs font-extrabold text-foreground">{editingId ? '개인 기록 수정' : '개인 기록 추가'}</h4>
                <button type="button" onClick={closeForm} disabled={formPending} className="text-[11px] text-muted-foreground">닫기</button>
              </div>
              <div className="grid grid-cols-2 gap-2">
                <label className="text-[10px] font-bold text-foreground space-y-1">
                  <span>시작일 *</span>
                  <input type="date" value={draft.startDate} onChange={(event) => setDraft((current) => ({ ...current, startDate: event.target.value }))} className="w-full p-2.5 rounded-xl border border-border bg-card text-xs" disabled={formPending} />
                </label>
                <label className="text-[10px] font-bold text-foreground space-y-1">
                  <span>종료일 (선택)</span>
                  <input type="date" value={draft.endDate || ''} onChange={(event) => setDraft((current) => ({ ...current, endDate: event.target.value || undefined }))} min={draft.startDate} className="w-full p-2.5 rounded-xl border border-border bg-card text-xs" disabled={formPending} />
                </label>
              </div>
              <fieldset className="space-y-2" disabled={formPending}>
                <legend className="text-[10px] font-bold text-foreground">증상 (선택)</legend>
                <div className="grid grid-cols-2 gap-2">
                  {CYCLE_SYMPTOMS.map((symptom) => (
                    <label key={symptom} className="flex items-center gap-2 p-2 rounded-xl bg-card border border-border text-[11px] text-foreground">
              ...
              <label className="text-[10px] font-bold text-foreground space-y-1 block">
                <span>메모 (선택)</span>
                <textarea value={draft.notes || ''} onChange={(event) => setDraft((current) => ({ ...current, notes: event.target.value }))} rows={3} className="w-full p-3 rounded-xl border border-border bg-card text-xs resize-none" disabled={formPending} />
              </label>
              {formError && <p className="text-[11px] text-destructive" role="alert">{formError}</p>}
              <div className="flex gap-2">
                {editingId && (
                  <button type="button" onClick={() => {
                    const entry = entries.find((item) => item.id === editingId);
                    if (entry) void deleteEntry(entry);
                  }} disabled={formPending || deletePendingId === editingId} className="px-3 py-2.5 rounded-xl border border-destructive/30 text-destructive text-xs font-bold disabled:opacity-50 min-h-[42px]">
          ...
          <div className="rounded-2xl border border-border p-4 space-y-3">
            <div>
              <h4 className="text-xs font-extrabold text-foreground">평균 길이 설정</h4>
              <p className="text-[10px] text-muted-foreground mt-0.5">저장된 평균 기간: {periodLength}일</p>
            </div>
            <div className="grid grid-cols-2 gap-2">
              <label className="text-[10px] font-bold text-foreground space-y-1">
                <span>평균 주기 길이</span>
                <input type="number" min={CYCLE_LENGTH_MIN} max={CYCLE_LENGTH_MAX} value={cycleLengthDraft} onChange={(event) => setCycleLengthDraft(Number(event.target.value))} className="w-full p-2.5 rounded-xl border border-border bg-card text-xs" disabled={settingsPending} />
              </label>
              <label className="text-[10px] font-bold text-foreground space-y-1">
                <span>평균 기간</span>
                <input type="number" min={PERIOD_LENGTH_MIN} max={PERIOD_LENGTH_MAX} value={periodLengthDraft} onChange={(event) => setPeriodLengthDraft(Number(event.target.value))} className="w-full p-2.5 rounded-xl border border-border bg-card text-xs" disabled={settingsPending} />
              </label>
            </div>
            <p className="text-[9px] text-muted-foreground">주기 {CYCLE_LENGTH_MIN}~{CYCLE_LENGTH_MAX}일 · 기간 {PERIOD_LENGTH_MIN}~{PERIOD_LENGTH_MAX}일</p>
            {settingsError && <p className="text-[11px] text-destructive" role="alert">{settingsError}</p>}
            <button type="button" onClick={() => void saveSettings()} disabled={settingsPending} className="w-full flex items-center justify-center gap-1.5 py-2.5 rounded-xl bg-foreground text-background text-xs font-bold disabled:opacity-50 min-h-[42px]">
