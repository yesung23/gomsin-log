  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-bold text-navy">우리의 디데이</h2>
      </div>

      {/* Connection Days Card */}
      <div 
        className="bg-gradient-to-br from-lilac to-coral/20 p-5 rounded-2xl border border-white relative overflow-hidden flex flex-col justify-center cursor-pointer"
        onClick={() => {
          if (!anniversaryDate) navigate('/my');
        }}
      >
        <Heart className="w-24 h-24 text-white/40 absolute -right-4 -bottom-4 rotate-12" />
        <p className="text-navy font-medium mb-1 text-xs">우리가 함께한 지</p>
        ...
        {anniversaryDate && (
          <div className="pt-3 border-t border-white/50 text-[11px] text-navy flex items-center gap-1.5 font-bold">
      ...
      <div 
        ...
        className="bg-gray-50 rounded-2xl border border-gray-100 p-4 flex items-center justify-between active:bg-gray-100 transition-colors cursor-pointer"
      >
        <div className="flex items-center gap-3">
          ...
          <div className="text-left">
            <div className="font-bold text-navy text-sm">복무 현황 · D-Day</div>
            <div className="text-[11px] text-gray-500 mt-0.5">전역일, 복무율, 연락 가능 시간</div>
          </div>
        </div>
      </div>
    </div>
