...
export function TodayLogWidget() {
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (path) {
        ...
        setAttachments(prev => [...prev, { type: 'photo', name: file.name, url, path }]); 
  ...
  return (
    <div className="flex flex-col">
      <h2 className="text-lg font-bold text-navy mb-4">오늘의 기록</h2>
      ...
      <div className="grid grid-cols-3 gap-2">
        ...
        <button
          ...
          className="flex flex-col items-center justify-center py-4 px-2 rounded-2xl bg-gray-50 border border-gray-100 text-gray-700 font-semibold text-sm active:scale-95 transition min-h-[60px]"
        >
        ...
        <button
          ...
          className="flex flex-col items-center justify-center py-4 px-2 rounded-2xl bg-gray-50 border border-gray-100 text-gray-700 font-semibold text-sm active:scale-95 transition min-h-[60px]"
        >
          <Send size={22} className="mb-1 text-gray-400" />
          <span>한줄남기기</span>
        </button>
      </div>
      ...
      {/* Input Composer */}
      {showInputCard && (
        <div className="mt-4 p-4 rounded-2xl bg-white border border-gray-100 shadow-sm animate-fade-in space-y-3">
          <div className="flex items-center justify-between pb-3 border-b border-gray-100">
            ...
            <button
              ...
              className="text-xs text-gray-500 px-2 py-1 bg-gray-50 rounded-md"
            >
              닫기
            </button>
          </div>
          ...
          <textarea
            ...
            className="w-full h-24 bg-gray-50 rounded-xl p-3 text-sm outline-none resize-none"
          />
                    ...
                    <button
                      ...
                      className={`px-3 py-2 rounded-xl text-xs font-bold transition flex items-center gap-1.5 active:scale-95 ${
                        isSelected
                          ...
                          : 'bg-white text-gray-700 border border-gray-200 hover:border-coral/40'
                      }`}
                    >
          ...
          <div className="pt-2 flex items-center justify-between">
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-bold flex items-center gap-1 ${
                isPrivate ? 'bg-amber-100 text-amber-800' : 'bg-gray-100 text-gray-500'
              }`}
            >
      ...
      <div className="mt-6">
        ...
        {todayRecords.length === 0 ? (
          <div className="text-center py-6 bg-gray-50 rounded-2xl border border-dashed border-gray-200">
            <p className="text-xs text-gray-400">아직 남겨진 기록이 없어요. 소중한 순간을 남겨보세요!</p>
          </div>
        ) : (
          <div className="space-y-2.5">
            {todayRecords.slice(-3).map(r => {
              ...
              return (
                <div key={r.id} className="bg-gray-50 rounded-xl p-3.5 flex flex-col gap-1.5 border border-gray-100">
                  <div className="flex items-center justify-between text-xs">
                    <span className="font-bold text-navy">{r.time}</span>
                  ...
                  <div className="text-sm text-gray-700">{r.log || (r.attachments ? '📷 사진 기록' : '리액션')}</div>
                </div>
              );
            })}
