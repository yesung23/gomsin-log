...
describe('V5-B - incomplete Apple login stays off in release artifacts', () => {
  const VALID_RELEASE = {
    ...VALID,
  ...
  it('allows exact true in an ordinary non-release build for local testing', () => {
    expect(() => validateBuildEnvironment({
      ...VALID,
      VITE_APPLE_LOGIN_ENABLED: 'true',
  ...
  it.each([
    ...
    expect(() => validateBuildEnvironment({
      ...
      ...releaseSignal,
      VITE_APPLE_LOGIN_ENABLED: 'true',
    })).toThrow(/APPLE_LOGIN_RELEASE_HOLD/);
  });

  it.each([undefined, '', 'false', 'TRUE', '1', ' true ', 'true\n'])(
    'keeps release builds valid when the Apple flag is not exact true: %j',
    (flag) => {
      expect(() => validateBuildEnvironment({
        ...VALID_RELEASE,
        isRelease: true,
        VITE_APPLE_LOGIN_ENABLED: flag,
      })).not.toThrow();
    },
  );

  it('reports a static activation-review hold without echoing build secrets', () => {
    ...
    expect(thrown!.message).toMatch(/APPLE_LOGIN_RELEASE_HOLD/);
    expect(thrown!.message).toMatch(/separately reviewed activation commit/i);
...
describe('V5-IAP - unverified Apple IAP sales stay off in release artifacts', () => {
  const VALID_RELEASE = {
    ...VALID,
    VITE_SUPABASE_PUBLISHABLE_KEY: 'sb_publishable_release_test_token',
    VITE_LEGAL_OPERATOR_NAME: '테스트 운영자',
  ...
  it('allows exact true only in an ordinary development build for local StoreKit testing', () => {
    expect(() => validateBuildEnvironment({
      ...VALID,
      VITE_APPLE_IAP_SALE_ENABLED: 'true',
      buildMode: 'development',
    })).not.toThrow();
  });
  ...
  it.each([
    ...
    expect(() => validateBuildEnvironment({
      ...
      ...releaseSignal,
      VITE_APPLE_IAP_SALE_ENABLED: 'true',
    })).toThrow(/APPLE_IAP_SALE_RELEASE_HOLD/);
  });

  it.each([undefined, '', 'false', 'TRUE', '1', ' true ', 'true\n'])(
    'keeps release builds valid when the IAP flag is not exact true: %j',
    (flag) => {
      expect(() => validateBuildEnvironment({
        ...VALID_RELEASE,
        isRelease: true,
        VITE_APPLE_IAP_SALE_ENABLED: flag,
      })).not.toThrow();
    },
  );
});
...
describe('supabase/config.toml function verification configuration', () => {
  it('declares explicit verify_jwt configuration for all edge functions', () => {
    ...
    expect(config).toMatch(/\[functions\.apple-iap-sync\]\s*verify_jwt\s*=\s*true/);
    expect(config).toMatch(/\[functions\.apple-iap-notifications\]\s*verify_jwt\s*=\s*false/);
    expect(config).toMatch(/\[functions\.apple-iap-reconcile\]\s*verify_jwt\s*=\s*false/);
    expect(config).toMatch(/\[functions\.apple-iap-consumption\]\s*verify_jwt\s*=\s*false/);
  });
});
});
