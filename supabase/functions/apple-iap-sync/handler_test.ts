...
import { JWS_A, TOKEN_A, TOKEN_B, USER_A, transactionFixture } from '../_shared/appleIapTestFixtures.ts';

const request = (body: unknown, token = 'valid-user-token') => new Request('https://edge.test/apple-iap-sync', {
  method: 'POST',
...
Deno.test('apple-iap-sync: anon and invalid bearer are denied before JWS verification', async () => {
  const fixture = deps();
  let verifies = 0;
  fixture.value.verifyTransaction = async () => { verifies += 1; return transactionFixture(); };
  const response = await handleAppleIapSync(request({ signedTransactionJws: JWS_A }, 'invalid'), fixture.value);
...
Deno.test('apple-iap-sync: malformed and unverified JWS are rejected without ledger writes', async () => {
  ...
  const unverified = await handleAppleIapSync(request({ signedTransactionJws: 'bad.jws.value' }), fixture.value);
...
Deno.test('apple-iap-sync: oversized request bodies are rejected before verification', async () => {
  ...
  let verifies = 0;
  fixture.value.verifyTransaction = async () => { verifies += 1; return transactionFixture(); };
  const response = await handleAppleIapSync(request({ padding: 'x'.repeat(40_001) }), fixture.value);
...
Deno.test('apple-iap-sync: another account token cannot claim or finish a transaction', async () => {
  ...
  fixture.value.ingestTransaction = async () => { throw new Error('database binding mismatch'); };
  const response = await handleAppleIapSync(request({ signedTransactionJws: JWS_A }), fixture.value);
...
Deno.test('apple-iap-sync: read action returns server state without accepting client entitlement claims', async () => {
  const fixture = deps();
  const response = await handleAppleIapSync(request({ action: 'status', environment: 'Production', entitlements: ['forged'] }), fixture.value);
  const body = await response.json();
  assert.equal(response.status, 200);
...
Deno.test('apple-iap-sync: purchase token is product-scoped and remains unavailable when server sale is off', async () => {
  const fixture = deps();
  const open = await handleAppleIapSync(request({
    action: 'prepare',
    productId: 'app.gomsinlog.paper.season.spring.v1',
    environment: 'Sandbox',
  ...
  const closed = await handleAppleIapSync(request({
    action: 'prepare',
