...
Deno.test({
  name: 'issue-recovery-challenge: fails closed with 500 when server config is missing',
...
Deno.test({
  name: 'issue-recovery-challenge: origin gate rejects disallowed origins with 403',
  permissions: { net: true, env: true, read: true, run: true },
  fn: async () => {
    const entrypoint = await startEntrypoint(
      new URL('./index.ts', import.meta.url),
...
Deno.test({
  name: 'issue-recovery-challenge: unauthenticated caller receives 401 E_UNAUTHENTICATED and verifies admin headers',
  permissions: { net: true, env: true, read: true, run: true },
  fn: async () => {
    ...
    const entrypoint = await startEntrypoint(
      new URL('./index.ts', import.meta.url),
      {
        ...
        SUPABASE_SECRET_KEYS: JSON.stringify({ default: 'sb_secret_admin_issue' }),
      },
      { method: 'OPTIONS', headers: { Origin: ALLOWED } },
    );
    try {
      const response = await fetch(entrypoint.origin, {
        ...
        headers: {
          Origin: ALLOWED,
          Authorization: 'Bearer caller_user_token_456',
      ...
      assertEquals(authReq?.apikey, 'sb_secret_admin_issue', 'apikey must be injected admin key');
      assertEquals(authReq?.authorization, 'Bearer caller_user_token_456', 'caller user token must be preserved');
    } finally {
      await entrypoint.stop();
      await stub.stop();
    }
